<%-- 
    Document   : FoundationSem
    Created on : 16 Mar, 2017, 10:03:43 PM
    Author     : gaurav
--%>
                                ...
                                <div class="">
                                    <%
                                        ...
                                        if (request.getParameter("insertButton") != null) {
                                            try {
                                                ...
                                                PaperSem papersem = new PaperSem(con);
                                                String foundationSem[] = request.getParameterValues("sem");
                                                paper.setPaperID(Integer.parseInt(request.getParameter("papers")));
                                                papersem.setAcdemicYear(request.getParameter("acedamicYear"));
                                                papersem.setPapers(paper);

                                                int sem1[] = new int[foundationSem.length];

                                                for (int i = 0; i < sem1.length; i++) {
                                                    sem1[i] = Integer.parseInt(foundationSem[i]);
                                                    papersem.setSem(sem1[i]);
                                                    papersem.insertPaperSem();
                                                    
                                                }
                                                out.println("<div class=\"alert alert-success\" id=\"insertSuccess\">"
                                                            + "<strong>Success!</strong>  core sem added successfully!."
                                                            + "</div>");
                                            } catch (SQLException sqlexception) {
                                                out.println("<div class=\"alert alert-danger\" id=\"invalid\">"
                                                        + "<strong>Invalid!</strong> core sem already exists!."
                                                        + "</div>");
                                            ...
                                            paper.setPaperID(Integer.parseInt(request.getParameter("papersID")));
                                            papersem.setAcdemicYear(request.getParameter("academicYear"));
                                            papersem.setSem(Integer.parseInt(request.getParameter("semester")));
                                            papersem.setPapers(paper);
                                           
                                            try {
                                                papersem.deletePaperSem();
                                                out.println("<div class=\"alert alert-success\" id=\"insertSuccess\">"
                                                        + "<strong>Success!</strong>  core sem deleted successfully!."
                                                        + "</div>");
                                            } catch (SQLException sqlexception) {
                                                out.println("<div class=\"alert alert-danger\" id=\"invalid\">"
                                    ...
                                    <form action="" method="post">
                                        <div class="col-md-12 card-style attendance-container " >
                                            <h3 class="text-center">ADD FOUNDATION SEM</h3>
                                            <div class="row">
                                                <div class="col-md-12">

                                                    <div class="col-md-4 col-md-offset-2">
                                                        <div class="form-group">
                                                            <label for="sel1">Papers:</label>
                                                            <select class="form-control" name="papers" id="papers">

                                                                <option disabled selected value>--select an option--</option>
                                                                <%
                                                                    Foundation foundation[] = Foundation.getAllFoundatioPapers(con);
                                                                    for (int i = 0; i < foundation.length; i++) {
                                                                        int paperID = foundation[i].getPaperID();
                                                                        out.println("<option value=" + paperID + "> " + foundation[i].getPaperName() + "</option>");
                                                                    }
                                                    ...
                                                    <div class="col-md-4" id="">
                                                        <div class="form-group" id="">
                                                            ...
                                                            <input type="text" Value="" class="form-control pull-right" placeholder="Acedamic Year" name="acedamicYear" id="acedamicYear" >
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="attend-scroll">
                                                <div class="col-md-12">
                                                    <div class="panel panel-success">
                                                        <h3 class="text-center">FOUNDATION SEM DIRECTORY</h3>
                                                        ...
                                                        <table class="table table-hover" id="task-table">
                                                            ...
                                                            <tbody>
                                                                <%                                                                  
                                                                    PaperSem papersems = new PaperSem(con);
                                                                    PaperSem papersem[] = papersems.getFoundationPaperSem();
                                                                    for (int i = 0; i < papersem.length; i++) {
                                                                        out.println("<form>");
                                                                        out.println("<tr>");
                                                                        out.println("<td>" + papersem[i].getPapers().getPaperName() + "</td>"
                                                                                + "<td style='visibility:hidden'><input type='hidden' name='papersID' value=" + papersem[i].getPapers().getPaperID() + "></td>"
                                                                                + "<td>" + papersem[i].getAcdemicYear() + "</td>"
                                                                                + "<td><input type='hidden' name='academicYear' value=" + papersem[i].getAcdemicYear() + "></td>"
                                                                                + "<td>semester " + papersem[i].getSem() + "</option>"
                                                                                + "<td><input type='hidden' name='semester' value=" + papersem[i].getSem() + "></td>");

                                                                        out.println("<td><input  type='submit' onclick='bootbox.confirm();' class='delete-btn' id='confirm' name='delete' value='delete'></td>");
                                                                        out.println("</tr>");
                                                                        out.println("</form>");
                                                                    }
